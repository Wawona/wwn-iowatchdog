...
| `wwn_watchdogd_job_restore` | enable + bootstrap + kickstart (no `-k`), retry until alive or timeout |
| Restore LaunchDaemon | Waits for `pgrep watchdogd` after bootstrap; exits non-zero if still dead |
